...
public record ChunkingSchema(
     ...
     * <p>
     * 2. Если размер контейнера > chunkSize + minChunkSize, то контейнер разбивается на n фрагментов.
     ...
     *
     * @param size         полный размер разбиваемого контейнера (байт)
    ...
    public long getOffsetChunk(int chunkNumber) {
        if (chunkNumber < FIRST_CHUNK_NUMBER || chunkNumber > count)
            throw new IndexOutOfBoundsException("Порядковый номер фрагмента выходит за диапазон допустимых значений");

        return (long) (chunkNumber - FIRST_CHUNK_NUMBER) * chunkSize;
    }
    ...
    public boolean isValid() {
        return size == (long) (count - 1) * chunkSize + lastChunkSize;
    }
}